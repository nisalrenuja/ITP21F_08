...
export default class SalaryReport extends Component {
  ...
  render() {
        ...
        const totalbasic2021 = this.state.salaries.reduce((totalbasic2021, salaries) => 
            totalbasic2021 += parseInt(salaries.basic,10), 0)
        //calculate total netsal
        const totalsalary2021 = this.state.salaries.reduce((totalsalary2021, salaries) => 
            totalsalary2021 += parseInt(salaries.basic +(salaries.OT_hrs * 120 + salaries.aws + salaries.bonus +salaries.total_earnings) -(salaries.nopay_leaves * 100 + salaries.total_deductions) ,10), 0)
        
        //count no of employeed who got paid
            const totalpaidcount = this.state.salaries.reduce((totalpaidcount, salaries) => {
        ...
        const sepPaidcount = this.state.salaries.reduce((sepPaidcount, salaries) => {
          if (salaries.salary_status.toLowerCase() === 'recieved' && salaries.pay_month.toLowerCase() === 'september' ) {
              return sepPaidcount =+ (salaries.basic);
          } 
          return sepPaidcount;
      }, 0);
        const sepnotPaidcount = this.state.salaries.reduce((sepnotPaidcount, salaries) => {
          if (salaries.salary_status.toLowerCase() === 'not recieved' && salaries.pay_month.toLowerCase() === 'september' ) {
              return sepnotPaidcount + (salaries.basic);
          } 
          return sepnotPaidcount;
      }, 0);
        const augPaidcount = this.state.salaries.reduce((augPaidcount, salaries) => {
          if (salaries.salary_status.toLowerCase() === 'recieved' && salaries.pay_month.toLowerCase() === 'august' ) {
              return augPaidcount + (salaries.basic);
          } 
          return augPaidcount;
      }, 0);
        const augnotPaidcount = this.state.salaries.reduce((augnotPaidcount, salaries) => {
          if (salaries.salary_status.toLowerCase() === 'not recieved' && salaries.pay_month.toLowerCase() === 'august' ) {
